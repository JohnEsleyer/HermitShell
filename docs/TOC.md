...
- Agent/controller interaction prefers a deterministic XML-tag contract (`<thought>`, `<message>`, `<terminal>`, `<action>`). JSON/labeled parsing remains for compatibility, and `panelActions` is legacy-only.
