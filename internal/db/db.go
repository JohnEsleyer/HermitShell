...
func (d *DB) migrate() error {
	...
	CREATE TABLE IF NOT EXISTS agents (
		...
		background TEXT NOT NULL DEFAULT 'doodle',
	...
	if err := d.addColumnIfNotExists("agents", "background", "TEXT NOT NULL DEFAULT 'doodle'"); err != nil {
		return err
	}
...
func (d *DB) CreateAgent(a *Agent) (int64, error) {
	res, err := d.db.Exec(`
		INSERT INTO agents (name, role, personality, provider, model, system_prompt, telegram_id, telegram_token, profile_pic, banner_url, background, tunnel_id, tunnel_url, allowed_users, container_id, status, platform, active)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, a.Name, a.Role, a.Personality, a.Provider, a.Model, a.Context, a.TelegramID, a.TelegramToken, a.ProfilePic, a.BannerURL, a.Background, a.TunnelID, a.TunnelURL, a.AllowedUsers, a.ContainerID, a.Status, a.Platform, a.Active)
	if err != nil {
		return 0, err
	}
...
func (d *DB) GetAgent(id int64) (*Agent, error) {
	a := &Agent{}
	err := d.db.QueryRow(`
		SELECT id, name, role, personality, provider, model, system_prompt, telegram_id, telegram_token, profile_pic, banner_url, background, tunnel_id, tunnel_url, allowed_users, container_id, status, platform, active, llm_api_calls, context_window, created_at, updated_at
		FROM agents WHERE id = ?
	`, id).Scan(&a.ID, &a.Name, &a.Role, &a.Personality, &a.Provider, &a.Model, &a.Context, &a.TelegramID, &a.TelegramToken, &a.ProfilePic, &a.BannerURL, &a.Background, &a.TunnelID, &a.TunnelURL, &a.AllowedUsers, &a.ContainerID, &a.Status, &a.Platform, &a.Active, &a.LLMAPICalls, &a.ContextWindow, &a.CreatedAt, &a.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return a, nil
...
func (d *DB) GetAgentByName(name string) (*Agent, error) {
	...
	`, name).Scan(&a.ID, &a.Name, &a.Role, &a.Personality, &a.Provider, &a.Model, &a.Context, &a.TelegramID, &a.TelegramToken, &a.ProfilePic, &a.BannerURL, &a.Background, &a.TunnelID, &a.TunnelURL, &a.AllowedUsers, &a.ContainerID, &a.Status, &a.Platform, &a.Active, &a.LLMAPICalls, &a.ContextWindow, &a.CreatedAt, &a.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return a, nil
...
func (d *DB) ListAgents() ([]*Agent, error) {
	rows, err := d.db.Query(`
		SELECT id, name, role, personality, provider, model, system_prompt, telegram_id, telegram_token, profile_pic, banner_url, background, tunnel_id, tunnel_url, allowed_users, container_id, status, platform, active, created_at, updated_at
	...
	for rows.Next() {
		a := &Agent{}
		if err := rows.Scan(&a.ID, &a.Name, &a.Role, &a.Personality, &a.Provider, &a.Model, &a.Context, &a.TelegramID, &a.TelegramToken, &a.ProfilePic, &a.BannerURL, &a.Background, &a.TunnelID, &a.TunnelURL, &a.AllowedUsers, &a.ContainerID, &a.Status, &a.Platform, &a.Active, &a.CreatedAt, &a.UpdatedAt); err != nil {
			return nil, err
		}
...
func (d *DB) UpdateAgent(a *Agent) error {
	_, err := d.db.Exec(`
		UPDATE agents SET name=?, role=?, personality=?, provider=?, model=?, system_prompt=?, telegram_id=?, telegram_token=?, profile_pic=?, banner_url=?, background=?, tunnel_id=?, tunnel_url=?, allowed_users=?, container_id=?, status=?, platform=?, active=?, updated_at=datetime('now')
		WHERE id=?
	`, a.Name, a.Role, a.Personality, a.Provider, a.Model, a.Context, a.TelegramID, a.TelegramToken, a.ProfilePic, a.BannerURL, a.Background, a.TunnelID, a.TunnelURL, a.AllowedUsers, a.ContainerID, a.Status, a.Platform, a.Active, a.ID)
	return err
}
